...
describe('site page manifest', () => {
  it('keeps canonical routes, main navigation, and footer links in one manifest boundary', () => {
    ...
    expect(canonicalRoutes).toContain('/contact')
    expect(canonicalRoutes).toContain('/download')
    expect(canonicalRoutes).not.toContain('/login')
    expect(canonicalRoutes).not.toContain('/demo')
    ...
    expect(footerRoutes).toContain('/download')
  })
})
